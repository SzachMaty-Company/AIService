...
class board():
    ...
    def move_piece(self, initial_position, final_position, promoted_piece="Q", save_peasant=False):
        if self.player == 0:
            promoted = False
            i, j = initial_position
            piece = self.current_board[i, j]
            if save_peasant and piece == "P" and abs(i - final_position[0]) > 1:
                letters = "abcdefgh"
                self.en_passant = letters[j] + str(7-(i-2))
            else:
                self.peasant = "-"
