...
class UCTNode():
    def __init__(self, game, move, parent=None, use_new=False):
    ...
    def select_leaf(self):
        ...
        #function change 8x8 chessboard array to fen
    def board_to_fen(self, game):
        fen = ""
        for i in range(8):
            empty = 0
            for j in range(8):
                if game.current_board[i][j] == " ":
                    empty += 1
                else:
                    if empty != 0:
                        fen += str(empty)
                        empty = 0
                    fen += game.current_board[i][j]
            if empty != 0:
                fen += str(empty)
            if i != 7:
                fen += "/"
        ...
        castling = ""
        if game.K_move_count == 0:
            if game.R2_move_count == 0:
                castling += "K"
            if game.R1_move_count == 0:
                castling += "Q"
        if game.k_move_count == 0:
            if game.r2_move_count == 0:
                castling += "k"
        ...
        fen += " " + str(game.no_progress_count) + " " + str(game.move_count)
    ...
    def expand(self, child_priors):
        ...
        c_p = child_priors
        if self.use_new:
            fen = self.board_to_fen(self.game)
            board = chess.Board(fen)
            y = board.generate_legal_moves()
            y = [(x.from_square, x.to_square, x.promotion) for x in y]
            y = [[((63-from_square)//8,from_square%8), ((63-to_square)//8,to_square%8), {5: None, 4: "rook", 3: "bishop", 2: "knight", None: None}[promotion]] for (from_square, to_square, promotion) in y]
        ...
        #
        #     builder = np.array(builder).reshape(8, 8)
        #     print(builder)
        #     print(sorted(y))
        #     print(sorted(mvs))
        for action in y:
...
def UCT_search(game_state, num_reads, net, use_new=False):
    root = UCTNode(game_state, move=None, parent=DummyNode(), use_new=use_new)
...
def do_decode_n_move_pieces(board, move):
    i_pos, f_pos, prom = ed.decode_action(board, move)
    for i, f, p in zip(i_pos, f_pos, prom):
        board.move_piece(i, f, p, True)  # move piece to get next board state s
        a, b = i
        c, d = f
        if board.current_board[c, d] in ["K", "k"] and abs(
                d - b) == 2:  # if king moves 2 squares, then move rook too for castling
...
def MCTS_self_play(chessnet, num_games, cpu):
    for idxx in range(0, num_games):
        ...
        while checkmate == False and current_board.move_count <= 100:
            ...
            # t = time.time()
            best_move, root = UCT_search(current_board, 40, chessnet, True)
