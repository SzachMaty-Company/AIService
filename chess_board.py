...
class Board(BaseBoard):
        ...
        for from_square in scan_reversed(capturers):
            ...
            for to_square in scan_reversed(targets):
                if square_rank(to_square) in [0, 7]:
                    yield (from_square, to_square, QUEEN, None)
                    yield (from_square, to_square, ROOK, None)
                    yield (from_square, to_square, BISHOP, None)
                    yield (from_square, to_square, KNIGHT, None)
                else:
                    yield (from_square, to_square, None, None)
    ...
    def is_en_passant(self, move: tuple) -> bool:
        ...
        return (self.ep_square == move[TO_SQUARE] and
                ...
                not self.occupied & BB_SQUARES[move[FROM_SQUARE]])
    ...
    def _generate_evasions(self, king: Square, checkers: Bitboard, from_mask: Bitboard = BB_ALL, to_mask: Bitboard = BB_ALL) -> Iterator[tuple]:
        ...
        if BB_SQUARES[king] & from_mask:
            for to_square in scan_reversed(BB_KING_ATTACKS[king] & ~self.occupied_co[self.turn] & ~attacked & to_mask):
                yield (king, to_square, None, None)
    ...
    def _from_chess960(self, chess960: bool, from_square: Square, to_square: Square, promotion: Optional[PieceType] = None, drop: Optional[PieceType] = None) -> tuple:
        if not chess960 and promotion is None and drop is None:
            if from_square == E1 and self.kings & BB_E1:
                if to_square == H1:
                    return (E1, G1, None, None)
                elif to_square == A1:
                    return (E1, C1, None, None)
            elif from_square == E8 and self.kings & BB_E8:
                if to_square == H8:
                    return (E8, G8, None, None)
                elif to_square == A8:
                    return (E8, C8, None, None)
